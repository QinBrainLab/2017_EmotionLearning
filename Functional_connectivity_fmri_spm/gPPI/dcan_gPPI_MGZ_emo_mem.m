function dcan_gPPI_MGZ_emo_mem(Config_File)
...
for i_roi = 1:num_roi_file
  ...
  for i_subj = 1:num_subj
    ...
    subject_gPPI_stats_dir = fullfile(data_server_stats, year_id, subjects{i_subj}, ...
      'fmri/stats_spm8', [stats_folder, '_gPPI_h18_l18_0.01_100']); % edited by genghaiyang
    ...
    P.Contrasts(1).name='negative_for';
    P.Contrasts(1).left={'neg_forgot_type'};
 	P.Contrasts(1).right={'none'};
    P.Contrasts(1).STAT='T';
    P.Contrasts(1).Weighted=0;
    %P.Contrasts(2).MinEvents=5;
    P.Contrasts(2).name='neutral_for';
    P.Contrasts(2).left={'neu_forgot_type'};
 	P.Contrasts(2).right={'none'};
    P.Contrasts(2).STAT='T';
    P.Contrasts(2).Weighted=0;
    %P.Contrasts(3).MinEvents=5;
    P.Contrasts(3).name='negative_HC';
    P.Contrasts(3).left={'neg_remembered_HC_type'};
 	P.Contrasts(3).right={'none'};
    P.Contrasts(3).STAT='T';
    P.Contrasts(3).Weighted=0;
    %P.Contrasts(4).MinEvents=5;
    P.Contrasts(4).name='neutral_HC';
    P.Contrasts(4).left={'neu_remembered_HC_type'};
 	P.Contrasts(4).right={'none'};
    P.Contrasts(4).STAT='T';
    P.Contrasts(4).Weighted=0;
    %P.Contrasts(5).MinEvents=5;
    P.Contrasts(5).name='neg_neu_for';
    P.Contrasts(5).left={'neg_forgot_type'};
 	P.Contrasts(5).right={'neu_forgot_type'};
    P.Contrasts(5).STAT='T';
    P.Contrasts(5).Weighted=0;
    %P.Contrasts(6).MinEvents=5;
    P.Contrasts(6).name='neg_neu_HC';
    P.Contrasts(6).left={'neg_remembered_HC_type'};
 	P.Contrasts(6).right={'neu_remembered_HC_type'};
    P.Contrasts(6).STAT='T';
    P.Contrasts(6).Weighted=0;
    
    % make T contrast and put i into P. added by genghaiyang
%     P.Contrasts(1).left={'neg'};
%  	P.Contrasts(1).right={'none'};
%     P.Contrasts(1).STAT='T';
%     P.Contrasts(1).Weighted=0;
%     %P.Contrasts(1).MinEvents=5;
%     P.Contrasts(1).name='neg';
%     
%     P.Contrasts(2).left={'neu'};
%  	P.Contrasts(2).right={'none'};
%     P.Contrasts(2).STAT='T';
%     P.Contrasts(2).Weighted=0;
%     %P.Contrasts(2).MinEvents=5;
%     P.Contrasts(2).name='neu';
% 
%     P.Contrasts(3).left={'unc'};
%  	P.Contrasts(3).right={'none'};
%     P.Contrasts(3).STAT='T';
%     P.Contrasts(3).Weighted=0;
%     %P.Contrasts(3).MinEvents=5;
%     P.Contrasts(3).name='unc';
%     
%     P.Contrasts(4).left={'neg'};
%  	P.Contrasts(4).right={'neu'};
%     P.Contrasts(4).STAT='T';
%     P.Contrasts(4).Weighted=0;
%     P.Contrasts(4).name='neg_neu';
%     
%     P.Contrasts(5).left={'unc'};
%  	P.Contrasts(5).right={'neu'};
%     P.Contrasts(5).STAT='T';
%     P.Contrasts(5).Weighted=0;
%     P.Contrasts(5).name='unc_neu';
%     
%     P.Contrasts(6).left={'unc'};
%  	P.Contrasts(6).right={'neg'};
%     P.Contrasts(6).STAT='T';
%     P.Contrasts(6).Weighted=0;
%     P.Contrasts(6).name='unc_neg'; 
