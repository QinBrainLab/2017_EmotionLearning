...
DataPath2='/fmri/EL/task_design/taskdesign_conspo2_EL_2s.m';
...
remembered_LC_type=find(Type==2|Type==6);
neg_forgot_type=find(Type==1|Type==3);
neu_forgot_type=find(Type==5|Type==7);
neg_remembered_HC_type=find(Type==4);
neu_remembered_HC_type=find(Type==8);

[size_face_type,t1]=size(face_type);
[size_remembered_LC_type,t2]=size(remembered_LC_type);
[size_neg_forgot_type,t3]=size(neg_forgot_type);
[size_neu_forgot_type,t4]=size(neu_forgot_type);
[size_neg_remembered_HC_type,t5]=size(neg_remembered_HC_type);
[size_neu_remembered_HC_type,t6]=size(neu_remembered_HC_type);

        for i=1:size_face_type
             onsets1(i)=OnsetTime_stim1(face_type(i));
        end
        
        for i=1:size_remembered_LC_type
             onsets2(i)=OnsetTime_stim2(remembered_LC_type(i));
        end

        for i=1:size_neg_forgot_type
             onsets3(i)=OnsetTime_stim2(neg_forgot_type(i));
        end
        for i=1:size_neu_forgot_type
             onsets4(i)=OnsetTime_stim2(neu_forgot_type(i));
        end
     
         for i=1:size_neg_remembered_HC_type
             onsets5(i)=OnsetTime_stim2(neg_remembered_HC_type(i));
        end
        for i=1:size_neu_remembered_HC_type
             onsets6(i)=OnsetTime_stim2(neu_remembered_HC_type(i));
        end

onsets1=onsets1/1000;
onsets2=onsets2/1000;
onsets3=onsets3/1000;
onsets4=onsets4/1000;
onsets5=onsets5/1000;
onsets6=onsets6/1000;
% 
% if onsets1(1)<0
...
fprintf(fp, '%s\n','names{2}     = [''remembered_LC_type''];');
fprintf(fp, '%s','onsets{2}    = [');
for i =1:size(remembered_LC_type)
...
fprintf(fp, '%s\n','names{3}     = [''neg_forgot_type''];');
fprintf(fp, '%s','onsets{3}    = [');
for i =1:size(neg_forgot_type)
...
fprintf(fp, '%s\n','names{4}     = [''neu_forgot_type''];');
fprintf(fp, '%s','onsets{4}    = [');
for i =1:size(neu_forgot_type)
    fprintf(fp, '%.3f', onsets4(i));
    fprintf(fp, '%s', ',');
end
fprintf(fp, '%s\n','];');
fprintf(fp, '%s','durations{4} = [');
for i =1:size(neu_forgot_type)
    fprintf(fp, '%.3f', 2);
    fprintf(fp, '%s', ',');
end
...
fprintf(fp, '%s\n','names{5}     = [''neg_remembered_HC_type''];');
fprintf(fp, '%s','onsets{5}    = [');
for i =1:size(neg_remembered_HC_type)
    fprintf(fp, '%.3f', onsets5(i));
    fprintf(fp, '%s', ',');
end
fprintf(fp, '%s\n','];');
fprintf(fp, '%s','durations{5} = [');
for i =1:size(neg_remembered_HC_type)
    fprintf(fp, '%.3f', 2);
    fprintf(fp, '%s', ',');
end
...
fprintf(fp, '%s\n','names{6}     = [''neu_remembered_HC_type''];');
fprintf(fp, '%s','onsets{6}    = [');
for i =1:size(neu_remembered_HC_type)
    fprintf(fp, '%.3f', onsets6(i));
    fprintf(fp, '%s', ',');
end
fprintf(fp, '%s\n','];');
fprintf(fp, '%s','durations{6} = [');
for i =1:size(neu_remembered_HC_type)
    fprintf(fp, '%.3f', 2);
    fprintf(fp, '%s', ',');
end
